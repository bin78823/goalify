    ...
    (set, get) => ({
      ...
      openTab: (tabData) => {
        ...
        if (tabData.projectId) {
          const existingTab = tabs.find(
            (t) => t.projectId === tabData.projectId,
          );
          if (existingTab) {
        ...
        // 子任务看板检查是否已存在
        if (tabData.type === "subtask" && tabData.taskId) {
          const existingSubtaskTab = tabs.find(
            (t) => t.type === "subtask" && t.taskId === tabData.taskId,
          );
          if (existingSubtaskTab) {
            set({ activeTabId: existingSubtaskTab.id });
            return existingSubtaskTab.id;
          }
        }
      ...
      syncWithProjects: (projectIds) => {
        ...
        if (
          ...
          activeTab.projectId &&
          !projectIds.includes(activeTab.projectId)
        ) {
          newActiveId = "home";
        }

...
pub fn create_project(conn: &Connection, name: &str, description: &str, start_date: &str, end_date: &str) -> SqliteResult<Project> {
    let id = uuid::Uuid::new_v4().to_string();
    let now = chrono::Utc::now().to_rfc3339();
    
    conn.execute(
        "INSERT INTO projects (id, name, description, start_date, end_date, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
...
pub fn get_all_projects(conn: &Connection) -> SqliteResult<Vec<Project>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, description, start_date, end_date, created_at, updated_at FROM projects ORDER BY created_at DESC"
    )?;
...
pub fn update_project(conn: &Connection, id: &str, name: Option<&str>, description: Option<&str>, start_date: Option<&str>, end_date: Option<&str>) -> SqliteResult<Option<Project>> {
    let now = chrono::Utc::now().to_rfc3339();
    ...
    if let Some(description) = description {
        conn.execute("UPDATE projects SET description = ?1, updated_at = ?2 WHERE id = ?3", params![description, now, id])?;
    }
    if let Some(start_date) = start_date {
        conn.execute("UPDATE projects SET start_date = ?1, updated_at = ?2 WHERE id = ?3", params![start_date, now, id])?;
    }
    if let Some(end_date) = end_date {
        conn.execute("UPDATE projects SET end_date = ?1, updated_at = ?2 WHERE id = ?3", params![end_date, now, id])?;
    }
...
pub fn update_task(
    ...
    if let Some(project_id) = project_id {
        conn.execute("UPDATE tasks SET project_id = ?1, updated_at = ?2 WHERE id = ?3", params![project_id, now, id])?;
    }
    if let Some(name) = name {
        conn.execute("UPDATE tasks SET name = ?1, updated_at = ?2 WHERE id = ?3", params![name, now, id])?;
    }
    if let Some(description) = description {
        conn.execute("UPDATE tasks SET description = ?1, updated_at = ?2 WHERE id = ?3", params![description, now, id])?;
    }
    if let Some(start_date) = start_date {
        conn.execute("UPDATE tasks SET start_date = ?1, updated_at = ?2 WHERE id = ?3", params![start_date, now, id])?;
    }
    if let Some(end_date) = end_date {
        conn.execute("UPDATE tasks SET end_date = ?1, updated_at = ?2 WHERE id = ?3", params![end_date, now, id])?;
    }
    if let Some(progress) = progress {
        conn.execute("UPDATE tasks SET progress = ?1, updated_at = ?2 WHERE id = ?3", params![progress, now, id])?;
    }
    if let Some(dependencies) = dependencies {
        conn.execute("UPDATE tasks SET dependencies = ?1, updated_at = ?2 WHERE id = ?3", params![dependencies, now, id])?;
    }
    if let Some(is_milestone) = is_milestone {
        let is_milestone_int = if is_milestone { 1 } else { 0 };
        conn.execute("UPDATE tasks SET is_milestone = ?1, updated_at = ?2 WHERE id = ?3", params![is_milestone_int, now, id])?;
    }
    if let Some(color) = color {
        conn.execute("UPDATE tasks SET color = ?1, updated_at = ?2 WHERE id = ?3", params![color, now, id])?;
    }

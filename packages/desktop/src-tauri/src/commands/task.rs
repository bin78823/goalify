...
#[tauri::command]
pub fn get_tasks_by_project(state: State<AppState>, project_id: String) -> Result<Vec<Task>, String> {
    let conn = state.db.lock().map_err(|e: std::sync::PoisonError<_>| e.to_string())?;
...
#[tauri::command]
pub fn update_task(state: State<AppState>, request: UpdateTaskRequest) -> Result<Option<Task>, String> {
    let conn = state.db.lock().map_err(|e: std::sync::PoisonError<_>| e.to_string())?;
    db::update_task(
        &conn,
        &request.id,
